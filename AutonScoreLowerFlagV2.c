...
void driveStraightDistance(int inches, int masterPower, int wheelRadius, float gearRatio)	{
  ...
  int leftTotalTicks = 0;//This will count up the total left encoder ticks despite the fact that the encoders are constantly reset.
  ...
  while(timerBool)	{
  	...
  	leftTotalTicks += SensorValue[leftEncoder];
  	rightTotalTicks += SensorValue[rightEncoder];
  	leftError = tickGoal - leftTotalTicks;
  	rightError = tickGoal - rightTotalTicks;
  	masterPower = leftError * driveKp;
  	slavePower = rightError * driveKp;
